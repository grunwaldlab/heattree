#' An example tree in "phylo" format
#'
#' The parsed version of the included "example_tree_1.treefile".
#'
#' @name example_tree_1
#' @format `ape::phylo`
#' @source From a core gene phylogeny of publicly available sequences
#' @keywords data
NULL

#' Example metadata for the tips of `example_tree_1`
#'
#' The parsed version of the included "example_metadata_1".
#'
#' @name example_metadata_1
#' @format `data.frame`
#' @source From a core gene phylogeny of publicly available sequences
#' @keywords data
NULL


#' A phylogeny from Bansal et al. 2021
#'
#' A core gene phylogeny from Bansal et al. 2021
#' (https://doi.org/10.1016/j.ygeno.2021.09.021) included for use as an example
#' data set. This is the parsed version of the included "bansal_2021_tree.nwk"
#' file.
#'
#' @name bansal_2021_tree
#' @format `ape::phylo`
#' @source Bansal, K., Kumar, S., Kaur, A., Singh, A. and Patil, P.B., 2021.
#'   Deep phylo-taxono genomics reveals Xylella as a variant lineage of plant
#'   associated Xanthomonas and supports their taxonomic reunification along
#'   with Stenotrophomonas and Pseudoxanthomonas. Genomics, 113(6),
#'   pp.3989-4003.
#' @keywords data
NULL

#' Isolate metadata from Bansal et al. 2021
#'
#' Isolate metadata from Bansal et al. 2021
#' (https://doi.org/10.1016/j.ygeno.2021.09.021) included for use as an example
#' data set. This is the parsed version of the included "bansal_2021_metadata.tsv"
#' file.
#'
#' @name bansal_2021_metadata
#' @format `data.frame`
#' @source Bansal, K., Kumar, S., Kaur, A., Singh, A. and Patil, P.B., 2021.
#'   Deep phylo-taxono genomics reveals Xylella as a variant lineage of plant
#'   associated Xanthomonas and supports their taxonomic reunification along
#'   with Stenotrophomonas and Pseudoxanthomonas. Genomics, 113(6),
#'   pp.3989-4003.
#' @keywords data
NULL


#' Isolate metadata from Weisberg et al. 2022
#'
#' Isolate metadata from Weisberg et al. 2022
#' (https://doi.org/10.1126/science.aba5256) included for use as an example data
#' set. This is the parsed version of the included "weisberg_2020_metadata.tsv"
#' file.
#'
#' @name weisberg_2020_metadata
#' @format `data.frame`
#' @source Alexandra J. Weisberg et al., Unexpected conservation and global
#'   transmission of agrobacterial virulence plasmids.Science368,eaba5256(2020)
#' @keywords data
NULL

#' A MLSA phylogeny from Weisberg et al. 2022
#'
#' A MLSA phylogeny from Weisberg et al. 2022
#' (https://doi.org/10.1126/science.aba5256) included for use as an example
#' data set. This is the parsed version of the included "weisberg_2020_mlsa.tre"
#' file.
#'
#' @name weisberg_2020_mlsa
#' @format `ape::phylo`
#' @source Alexandra J. Weisberg et al., Unexpected conservation and global
#'   transmission of agrobacterial virulence plasmids.Science368,eaba5256(2020)
#' @keywords data
NULL

#' A BEAST phylogeny from Weisberg et al. 2022
#'
#' A BEAST phylogeny from Weisberg et al. 2022
#' (https://doi.org/10.1126/science.aba5256) included for use as an example
#' data set. This is the parsed version of the included "weisberg_2020_beast.tre"
#' file.
#'
#' @name weisberg_2020_beast
#' @format `ape::phylo`
#' @source Alexandra J. Weisberg et al., Unexpected conservation and global
#'   transmission of agrobacterial virulence plasmids.Science368,eaba5256(2020)
#' @keywords data
NULL
