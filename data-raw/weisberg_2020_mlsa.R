# Download original file
source_url <- 'https://raw.githubusercontent.com/osuchanglab/AgrobacteriumEvolutionManuscript/refs/heads/master/chromosome_phylogenies/MLSA.tre'
temp_path <- tempfile()
download.file(source_url, destfile = temp_path)

# Subset for tips that have metadata associated with them
source('data-raw/weisberg_2020_metadata.R')
weisberg_2020_mlsa <- ape::read.tree(temp_path)
shared_ids <- weisberg_2020_mlsa$tip.label[weisberg_2020_mlsa$tip.label %in% weisberg_2020_metadata$mlsa_node_id]
weisberg_2020_mlsa <- ape::keep.tip(weisberg_2020_mlsa, shared_ids)

# Save file and parsed version of data
ape::write.tree(weisberg_2020_mlsa, file = 'inst/extdata/weisberg_2020_mlsa.tre')
usethis::use_data(weisberg_2020_mlsa, overwrite = TRUE)
